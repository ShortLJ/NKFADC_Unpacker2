...
void EventProcessor::Run()
{
	...
	while()
	{
		...
		//uint64_t ref_lgt = timesorter->GetLGT(sid,mid,cha);
...
Event EventProcessor::ProcessEvent(vector<Sig> v_sig)
{
	...
	for (uint8_t idet=0; idet<Ndet; idet++)
	{
		...
		{
			v_crystal.push_back(HitCrystal(idet,v_sig_sort[0][idet],v_sig_sort[1][idet]));
		}
	}
	...
	for (uint8_t idet=0; idet<Ndet; idet++)
	{
		if (v_sig_sort[2][idet].size()+v_sig_sort[3][idet].size()+v_sig_sort[4][idet].size()>0)
		{
			v_x6.push_back(HitX6(idet,v_sig_sort[2][idet],v_sig_sort[3][idet],v_sig_sort[4][idet]));
		}
	}
